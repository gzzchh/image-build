...
def excelWrite(a, filesname, sheet):
    ...
    print('    添加行')
    url = r'https://graph.microsoft.com/v1.0/me/drive/root:/ServerReport/App' + \
        str(a)+r'/'+filesname+r':/workbook/tables/'+jsontxt['id']+r'/rows/add'
    rowsvalues = [[0]*4]*2
    for v1 in range(0, 2):
        for v2 in range(0, 4):
...
def taskWrite(a, taskname):
    ...
    print("    删除任务")
    url = r'https://graph.microsoft.com/v1.0/me/todo/lists/' + \
        listjson['id']+r'/tasks/'+taskjson['id']
    apiReq('delete', a, url)
    print("    删除任务列表")
...
def teamWrite(a, channelname):
    ...
    for teamcount in range(teamlist['@odata.count']):
        if teamlist['value'][teamcount]['displayName'] == channelname:
            ...
            url = r'https://graph.microsoft.com/v1.0/teams/' + \
                teamlist['value'][teamcount]['id']+r'/channels'
            jsontxt = json.loads(apiReq('post', a, url, json.dumps(data)))
            url = r'https://graph.microsoft.com/v1.0/teams/' + \
                teamlist['value'][teamcount]['id']+r'/channels/'+jsontxt['id']
            print("    删除team频道")
            apiReq('delete', a, url)
            # 删除teams
            print("    删除team")
            url = r'https://graph.microsoft.com/v1.0/groups/' + \
                teamlist['value'][teamcount]['id']
            apiReq('delete', a, url)
...
weather = req.get(r'http://wttr.in/'+city +
                  r'?format=4&?m', headers=headers).text
...
for _ in range(1, config['rounds']+1):
    ...
    for a in range(1, int(app_num)+1):
        ...
        print('账号 '+str(a))
        # 生成随机名称
        filesname = time.strftime(
            "%Y-%m-%d", time.localtime())+' 的服务器报告 '+str(random.randint(1, 600))+r'.xlsx'
        # 新建随机xlsx文件
        ...
        if config['allstart'] == 1 or 1 in choosenum:
            print('excel文件操作')
            excelWrite(a, filesname, '服务器编号 '+str(random.randint(1, 600)))
        ...
        if config['allstart'] == 1 or 4 in choosenum:
            print('onenote操作')
            onenoteWrite(a, '当日记录: '+str(random.randint(1, 600)))
        print('-')
